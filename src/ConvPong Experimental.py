...
save_path = 'models/pong_ConV.ckpt'

rel_path = "Conv_Pong.txt"
...
def write_to_file(filename, v):
    ...
    with open(filename, "a") as f:
      #  message = "episode: " + str(v[0]) + ' average_reward: ' + str(v[1]) + "\n"
        message =  str(v)[1:-1].strip('\'') + "\n"
...
def tf_policy_forward(x):  # x ~ [1,D]
   ...
   # h_pool2 = max_pool_2x2(h_conv2)
    ...
    #p = y_convgame.get_state().screen_buffer
 ...
 #   time.sleep(0.001)
    ...
    if done:
        ...
        if episode_number % 50 == 0:
            message =  "ep %d: reward: %1d, mean reward: %.3f" % (episode_number, reward_sum, running_reward)
